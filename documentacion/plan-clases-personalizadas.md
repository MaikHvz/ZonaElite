...
> **Estado:** 🟢 EN IMPLEMENTACIÓN. Fases 0–4 de código completas (suite 244 tests A–P en verde, `npm run build` OK). Pendiente: aplicar migración 009 en Supabase (SQL Editor) y prueba manual sandbox Flow (Fase 4). Antes de tocar código, leer `contexto/requisitos/clases-personalizadas-modulo-independiente.md` y ejecutar el SOP de `documentacion/guia-de-trabajo.md`.
...
- [ ] Aplicar en Supabase (SQL Editor). **⚠️ Acción manual pendiente.**
...
- [ ] Prueba manual sandbox Flow: compra pack → callback → pack activo con fechas Chile correctas. **⚠️ Acción manual pendiente.**
- [ ] 2 packs en paralelo no se cancelan; consumo manual → `agotada`; notificación `approved`. **⚠️ Acción manual pendiente.**
...
- [x] Confirmar `documentacion/squema-sql-actualizado.sql` = BD en producción.
...
|------|-------------|--------|-------|---------|
| 0 | BD: `personalized_plans` + `personalized_packs` + RLS + índices | ✅ Hecha (falta aplicar SQL en Supabase) | 2026-08-04 | Sección P (esquema/RLS) |
...
| 4 | E2E sandbox + suite completa + build | 🟡 Parcial (suite 244 A–P + build OK; falta sandbox manual) | 2026-08-04 | 244 A–P en verde |
| 5 | Docs post-implementación (flujo-modulos, requisitos-implementados, BRAIN) | 🟡 Parcial (flujo-modulos + requisitos-implementados; BRAIN/informe si aplica) | 2026-08-04 | Revisión manual |
