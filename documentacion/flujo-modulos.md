...
- **UI**:
  ...
  - Admin `admin/ventas`: tab "Solicitudes" con badge de pendientes; lista filtrable por estado; bot�n "Revisar" abre modal con usuario/email/RUT/concepto/monto/referencia, voucher (img o iframe PDF) + link "Ver comprobante completo", nota opcional y botones Rechazar/Aprobar.
  - `PaymentRow` (dashboard/pagos): `transferencia`+`pendiente` muestra badge "En revisi�n" + `commerce_order`; `rechazado` muestra `admin_note`. Dashboard home reutiliza el mismo componente.
