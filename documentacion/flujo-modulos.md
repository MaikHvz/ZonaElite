...
- **Rutas Principales**:
  ...
  - `/quienes-somos` (`src/app/quienes-somos/page.tsx`): **Quiénes Somos** — comienza con la sección interactiva y responsive del Director Técnico (`src/components/MaestroSection.tsx`, con visor lightbox para imágenes en alta resolución, slider y vista en cuadrícula de torneos internacionales y palmarés), seguida de la **Historia interactiva de la academia** (selector de disciplinas: Kenpo, Kickboxing y MMA, con lector por capítulos, línea de tiempo interactiva y barra de progreso de lectura). Contenido de historia en `src/components/history/stories.ts`; interactividad en `src/components/history/HistoryExplorer.tsx`. `/nosotros` redirige 301 a esta ruta. Incluye `GalleryCarousel`, filosofía, FAQ y schema LocalBusiness.
...
- **Sub-Módulos del Dashboard**:
  ...
  - **Cargas/Dependientes (`/dashboard/cargas/page.tsx`)**:
    ...
    - **Dirección**: el perfil del tutor tiene un campo `address` (`profiles.address`, migración 018). Al crear/editar una carga se puede indicar su propia `address` (`dependents.address`) o marcar "Usar la misma dirección que el tutor", que autocompleta desde `profiles.address`. `DependentCard` muestra la dirección si existe. En el panel admin (`CreateDependentModal`) el mismo checkbox autocompleta desde la dirección del tutor seleccionado.
    - **Datos físicos**: el perfil y las cargas registran `weight` (kg), `height` (cm) y `dominant_hand` (`'diestro'|'zurdo'`) — migración 019, CHECK constraints en BD. Se validan con `src/lib/medidas.ts` (`normalizeMedida`/`parseMedida`/`isValidPeso` 0–300/`isValidAltura` 0–250/`isValidDominantHand`). Se editan en `/perfil`, en los modales de carga (dashboard y admin) y en la card `PhysicalInfoCard` de la ficha médica (guarda por browser client `dependents.update` con `.eq("id").eq("tutor_id")`). `DependentCard` muestra la fila "Datos físicos".
  - **Fichas Médicas (`/dashboard/cargas/[id]/medico/page.tsx`)**: Utiliza `MedicalInfoCard.tsx` y `EmergencyContactCard.tsx` para hacer *upsert* en la tabla `medical_records` (alergias, lesiones, contactos) y `PhysicalInfoCard.tsx` para los datos físicos de la carga (`dependents.update` por browser client).
...
- **Módulo de Productos (Tienda) 🟢 IMPLEMENTADO (v1.4.0)**:
  - Públicos: `/productos` y `/productos/[id]`. Cada producto tiene botones "Agregar al carrito" y "Comprar ahora" (Comprar agrega 1 item y navega a `/carrito`).
  - **Carrito**: `src/context/CartContext.tsx` (persistencia en localStorage `ze_cart`) con `CartProvider` en `src/app/layout.tsx`; `Navbar` muestra badge con contador (desktop + menú móvil).
  - **Checkout (`src/app/carrito/page.tsx` + `POST /api/store/checkout`)**: valida items y stock (precio siempre del servidor), autenticado → datos del perfil; invitado → email + teléfono obligatorios, nombre opcional. Reserva stock atómico vía RPC `decrement_product_stock` (guarda `stock >= qty`, 409 si no hay), crea `product_orders` (`pendiente`, `reference = REF-ZE-prod-<ts><rand>`) + `order_items` (snapshot `unit_price`) + `payments` (`flow`, `pendiente`, `order_id`, `user_id` opcional) y llama `createFlowOrder` con `returnUrl = /tienda/confirmacion`. Si algo falla post-reserva, restaura stock con `increment_product_stock`. El error real del fallo se incluye como `detail` en la respuesta 500 (debug en producción).
  - **Confirmación**: `GET /api/store/order-status?token=<flow_token>` (público, admin client) resuelve la orden; `/tienda/confirmacion` página pública que hace polling cada 4s y muestra éxito (resumen + items) o fallo ("stock devuelto").
  - **Callback Flow (desacoplado)**: `confirmation`, `verify` y `force-confirm` incluyen `order_id` en el select; cuando `isStorePayment(payment)` (`order_id` + concepto `Tienda:`) llaman a `handleStorePaymentApproved` (confirma `product_orders → 'pagado'` + `sendProductReceiptEmail` al correo de contacto) o `handleStorePaymentRejected` (cancela la orden + restaura stock). No tocan membresías/tokens/QR; la notificación in-app `approved` solo aplica si hay `user_id`.
  - **Recibo**: `src/lib/email.ts` → `sendProductReceiptEmail` (mismo transporter/estilos); `src/lib/store.ts` resuelve el destinatario (`getOrderContactEmail`: user_id → profiles.email, si no guest_email) y envía best-effort.
  - **Admin**: tab "Órdenes de Tienda" en `/admin/ventas` (`GET`/`PATCH /api/store/admin/orders`): lista órdenes con items y acciones Enviar (desde `pagado`), Entregar (desde `pagado`/`enviado`) y Cancelar (desde cualquier estado no cancelado → restaura stock, idempotente). Filtro por tipo (Tienda / Membresías / Inscripciones / Personalizadas) en la pestaña Pagos.
  - **Dashboards de usuario**: `src/app/dashboard/tienda/page.tsx` ("Mis Compras de Tienda", tab "Mi Tienda" en `DashboardNav`) y las ventas aparecen en `/dashboard/pagos` con concepto `Tienda: ...`.
...
- **Sub-Módulos (Secciones en `/admin`)**:
  ...
  - **Ventas (`/admin/ventas/page.tsx`)**: Panel de métricas transaccionales con exportador de Excel tipo "Cartola" filtrado por Mes, Año e Histórico. Tres tabs: **Pagos** (con filtros por estado, método y **tipo** — Tienda/Membresías/Inscripciones/Personalizadas — y detalle expandible), **Solicitudes** (pagos manuales por transferencia: revisar voucher, aprobar/rechazar con nota) y **Órdenes de Tienda** (lista de `product_orders` con items y acciones Enviar/Entregar/Cancelar-restaura stock vía `PATCH /api/store/admin/orders`).
  ...
  - **Configuración (`/admin/configuracion/page.tsx`)**: Configuración global del sitio, imágenes, y gestión de `GalleryCarousel.tsx` (subidas en `ImageUpload.tsx`).
   ...
   - **Crear y Asignar Carga**: Botón "Crear y Asignar Carga" en `/admin/usuarios` que abre `CreateDependentModal`: se selecciona el usuario tutor (profile padre) y se ingresan los datos de la carga (nombre, RUT, fecha de nacimiento, categoría, dirección y datos físicos). `POST /api/admin/create-dependent` inserta en `dependents` y asegura el registro en `beneficiaries` (admin client, idempotente) para que la carga quede utilizable en checkout/membresías. Las cargas existentes se editan con `POST /api/admin/update-dependent` desde la misma tabla. Las filas de cargas tienen botón ojo "Ver Ficha" (`DataTable onView`/`canView`, solo con `_isDependent`) que abre `VerFichaModal` (solo lectura) con datos personales + físicos.
...
- **Suite de Pruebas de Producción (`scripts/test-flows.mjs`)**: Test runner standalone (Node 24+, sin toolchain extra) con **573 checks** (secciones A–AC): límites de zona horaria `America/Santiago`, scan estático de patrones riesgosos (`toISOString().split("T")[0]` y límites de mes), firma HMAC-SHA256 de Flow (create-order + callback) y verificación de `commerceOrder` (B-007), contratos de esquema/RLS (incl. `chile_today()` en policies, RPC `enroll_class` B-006, tokens atados a membresía B-010, RPCs consolidadas B-011, `user_notifications` documentada B-012), ciclo de vida de inscripción (B-004), vencimiento efectivo (B-001/B-009), atomicidad de membresías (B-002), alerta admin post-pago (B-008), sección AA de **tienda con carrito y checkout Flow (v1.4.0)**, sección AB de **perfil deportivo de alumnos (v1.5.0)** y sección AC de **adjunto de imagen en eventos (v1.8.0)**. Comando: `node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/test-flows.mjs`. Retorna exit 1 si reaparece cualquier bug de fechas o se rompe un contrato.
...
## 13. Adjunto de Imagen en Eventos (IMPLEMENTADO - v1.7.0)

> Estado: **implementado (2026-09-15)**, suite secciones A-AC en verde, `npx tsc --noEmit` limpio. Sin cambios de esquema (reusa `events.image`). Migraci�n `030_changelog_v1_8_0.sql` (seed de changelog v1.8.0, idempotente, **pendiente aplicar en Supabase (SQL Editor)**). Se completaron en el espejo `squema-sql-actualizado.sql` los seeds `v1.6.0` (migraci�n 026) y `v1.7.0` (Hist�rico de asistencias, migraci�n 029) que faltaban, y se agreg� el `v1.8.0`, para mantenerlo 1:1. Requisito en `contexto/requisitos/adjunto-imagen-eventos.md`.

**Prop�sito**: La imagen que el admin sube en `/admin/eventos` (para cualquier tipo de evento: torneo, graduaci�n, seminario o clase especial) es la portada del evento y, adem�s, queda disponible como **documento adjunto descargable** en la ficha p�blica `/eventos/[id]`.

- **Sin cambios de BD**: portada y adjunto provienen de la misma `events.image` (bucket `public`, carpeta `events/`). No se agregan columnas ni tablas.
- **`src/components/ImagePreviewModal.tsx`** (nuevo, cliente): lightbox a pantalla completa con la imagen en `object-contain`, botones **"Cerrar"** y **"Descargar"**, cierre por ESC / clic en el overlay, y lock de scroll. Para forzar la descarga de URLs de Supabase Storage agrega el par�metro `?download=<archivo>` (honra `Content-Disposition: attachment`); para URLs externas abre en pesta�a nueva.
- **`/eventos/[id]`**: nueva secci�n **"Adjunto"** bajo la descripci�n (solo si `event.image`). Renderiza la imagen como tarjeta clickeable ("Clic para ampliar y descargar") que abre el `ImagePreviewModal`.
- **`/admin/eventos`**: el label de `ImageUpload` ahora indica "Imagen del evento (ser� la portada y el adjunto descargable)".
- **Verificaci�n**: suite secci�n AC (migraci�n 030 idempotente, espejo 1:1 con seeds v1.6.0/v1.7.0/v1.8.0, componentes y wiring de UI) + `tsc`.
