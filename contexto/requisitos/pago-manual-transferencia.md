...
- **Estado:** ✅ **IMPLEMENTADO — fases 0-8 completadas.** Suite secciones A-U en verde (429 tests), `npx tsc --noEmit` limpio, `npm run build` OK. Migración `013_manual_payment_mode.sql` creada y espejada 1:1 en `documentacion/squema-sql-actualizado.sql`; **pendiente aplicar en Supabase (SQL Editor)**. Incluye feedback admin (badge en Ventas + banner con CTA), feedback usuario (badge en tab Pagos + banner + panel "Mis Solicitudes de Pago" con motivo de rechazo), **correos de revisión** que dirigen al usuario a su sección "Mis Solicitudes de Pago" y al admin a la tab "Solicitudes", y el **RUT visible y filtrable** en la sección Usuarios del panel admin.
